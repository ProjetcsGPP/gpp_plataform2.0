...
class AplicacoesListEndpointTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.app_zeus = _make_aplicacao("ZEUS", "Zeus Gestão", isshowinportal=False)
        cls.app_ares = _make_aplicacao("ARES", "Ares Controle", isshowinportal=False)
        cls.app_portal = _make_aplicacao("PORTAL_VIS", "Portal Visualização", isshowinportal=True)
    ...
    def test_T03_non_admin_returns_403(self):
        ...
        self._as_plain()
        with patch("common.permissions.IsPortalAdmin.has_permission", return_value=False):
            response = self.client.get(self.list_url)
    ...
    def test_T04_list_returns_only_non_portal_apps(self):
        ...
        # R-01 — nenhuma app de portal na lista
        for item in data:
